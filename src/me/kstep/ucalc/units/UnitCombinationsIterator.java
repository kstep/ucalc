...
class UnitCombinationsIterator implements Iterator {
	...
	private void removeFromAgenda() {
		ListIterator restAgendaIter = restAgenda.listIterator();
		ListIterator baseAgendaIter = baseAgenda.listIterator();
		
		while (restAgendaIter.hasNext() && baseAgendaIter.hasNext()) {
			Unit[] base = (Unit[]) baseAgendaIter.next();
			Unit[] rest = (Unit[]) restAgendaIter.next();
